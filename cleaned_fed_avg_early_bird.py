...
class FedAvg:
    ...
    def _get_data(
        self, root: str, n_clients: int, non_iid: int
        ...
        sampler = FederatedSampler(
            train_set, non_iid=non_iid, n_clients=n_clients
        )
    ...
    def train(self, pre_eb = True) -> None:
        ...
        early_bird_30 = EarlyBird(0.3) #to change the pruning %
        ...
        np.savetxt(os.path.join(self.args.save, 'record_30.txt'), epoch_acc_tracker, fmt = '%10.5f', delimiter=',')
    ...
    def count_zero_weights(self, s = ""): #sanity check, does not add to the logic of the code
            print(s)
    ...
    def actual_prune(self):
        ...
        for m in self.root_model.modules():
            if isinstance(m, nn.BatchNorm2d):
                ...
                index += size        
        y, _ = torch.sort(bn)
        thre_index = int(total * 0.3) #change for pruning ratio
        thre = y[thre_index]
      ...
      #count the number of zero channels: 
        print(cfg)
        self.count_zero_weights(s = "Before removing zero channels") 
        ...
        self.count_zero_weights(s="After removing zero channels")
