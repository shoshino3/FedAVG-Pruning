...
class FedAvg:
    ...
    def _train_client(
        self, root_model: nn.Module, train_loader: DataLoader, client_idx: int, keep_masks: List[torch.Tensor]
        ...
        # keep_masks = SNIP(model, 0.5, train_loader, device)  
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.args.n_epochs): # epochs := rounds
            ...
            client_idx = idx_clients[0] # use the first client for mask generation

            # Train clients
            self.root_model.train()
            
            model = copy.deepcopy(self.root_model)
            model.train()
            optimizer = torch.optim.SGD(
                model.parameters(), lr=self.args.lr, momentum=self.args.momentum
            )
            ...
            if (epoch + 1) % self.args.log_every == 0:
                ...
                with open("res_multi_mask_with_train_loss.txt", 'a') as results_file:
