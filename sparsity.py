...
from utils import Logger, density_weights, density_masks, plot_data, density_weights
...
class FedAvg:
    ...
    def _get_data(
        ...
        print("alpha = 0.5,pruning rate = 98%, client_epoch = 5")
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if args.pruning and args.pruning_side == "server":
                ...
                layer_density, layer_sparsity = density_weights(self.root_model)
                for layer in layer_density.keys():
                  layer_name = layer.__class__.__name__ 
                  # Generate a unique name for each layer
                  if layer not in layer_file_paths:
                      layer_index = len([key for key in layer_file_paths.keys() if key.__class__.__name__ == layer_name]) + 1
                      numbered_layer_name = f"{layer_name}_{layer_index}"
                      file_path = os.path.join(self.args.save, f'{numbered_layer_name}.txt')
                      layer_file_paths[layer] = file_path
                  else:
                      file_path = layer_file_paths[layer]
  
                  # Append density and sparsity information to the correct file
                  with open(file_path, 'a') as f:
                      f.write(f"{epoch + 1},{layer_sparsity[layer].cpu().item():.3e},{layer_density[layer].cpu().item():.3e}\n")
            ...
            if (epoch + 1) % self.args.log_every == 0:
                ...
                epoch_acc_tracker[epoch,1] = total_acc
                    ...
                    #np.savetxt(os.path.join(self.args.save, f'density_sparsity_epoch_{epoch + 1}.txt'), density_sparsity_array)
        ...
        np.savetxt(os.path.join(self.args.save, 'record_98_5.txt'), epoch_acc_tracker, fmt = '%10.5f', delimiter=',')
...
def arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--save', default='./logs_4', type=str, metavar='PATH',
